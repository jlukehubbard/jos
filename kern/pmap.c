...
page_free(struct PageInfo *pp)
{
	...
	if (pp -> pp_ref)
		panic("page_free: bad free, page still in use");
	if (pp -> pp_link)
		panic("page_free: double free, page was already in the freelist");
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(!(*p_pde & PTE_P))
	{
		...
		new_table->pp_ref++;
		*p_pde = page2pa(new_table) | PTE_P | PTE_W | PTE_U;
		page_table = (pte_t*) KADDR(page2pa((struct PageInfo*) new_table));
	}
	else page_table = KADDR(PTE_ADDR(*p_pde));
...
{
	// LAB 3: Your code here.
	uintptr_t address = (uint32_t) va;
	uintptr_t start = ROUNDDOWN(address, PGSIZE);
	uintptr_t end = ROUNDUP(address + len, PGSIZE);
