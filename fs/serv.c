...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if ((r = openfile_lookup(envid, req->req_fileid, &o))) return r;

	if ((r = file_read(o->o_file, ret->ret_buf, req->req_n, o->o_fd->fd_offset)) > 0) o->o_fd->fd_offset += r;
	return r;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	int x;
	struct OpenFile * file_o;
	if((x = openfile_lookup(envid, req->req_fileid, &file_o)) < 0)
	{
		return x;
	}
	if((x = file_write(file_o->o_file, req->req_buf, req->req_n, file_o->o_fd->fd_offset)))
	{
		if(x > 0)
		{
			file_o->o_fd->fd_offset += x;
		}
		return x;
	}
}
